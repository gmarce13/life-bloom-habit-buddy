...
const StatsSection: React.FC<StatsSectionProps> = ({ habits }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 transition-colors duration-200">
      <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-6">Estadísticas</h2>
      
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map((stat, index) => (
          <div key={index} className={`${stat.bgColor} rounded-xl p-4 text-center transition-colors duration-200`}>
            <div className={`inline-flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-gray-800 mb-2`}>
              <stat.icon className={`w-5 h-5 ${stat.color}`} />
            </div>
            <div className="font-bold text-lg text-gray-800 dark:text-gray-200">{stat.value}</div>
      ...
      <div>
        <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-3">Progreso de los últimos 7 días</h3>
          ...
          {weeklyProgress.map((progress, index) => {
            ...
            return (
              <div key={index} className="flex-1 flex flex-col items-center">
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-t-lg flex items-end" style={{ height: '80px' }}>
                  <div
                ...
                <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">{dayName}</div>
              </div>
            );
          })}
        </div>
